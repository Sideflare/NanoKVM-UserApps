#!/usr/bin/env python3
"""
PicoClaw NanoKVM Screen App  v1.1.0
────────────────────────────────────
Consolidated Agent/Voice experience + QR Login.
"""

import os, sys, time, json, threading, mmap, textwrap, socket
import numpy as np
...
# ── Pages ──────────────────────────────────────────────────────────────────
PG_HOME, PG_AGENT, PG_SKILLS, PG_AUTH, PG_SETUP, PG_STATUS = range(6)
HOME_TARGETS = [PG_AGENT, PG_SKILLS, PG_AUTH, PG_SETUP, PG_STATUS]
HOME_LABELS  = ["Agent Chat", "Skills", "Login / Auth", "Setup", "Status"]
HOME_ICONS   = [">_ ", "## ", "[K]", "{} ", " i "]
...
class App:
    ...
    def _stats_loop(self):
        while self.running:
            ...
            if self.page in (PG_HOME, PG_STATUS, PG_SETUP): self._dirty = True
            time.sleep(2)
    ...
    def goto(self, page):
        ...
        elif page == PG_AUTH: self._gen_auth_qr()

    def _gen_auth_qr(self):
        ip = self.stats.get('ip', '127.0.0.1')
        url = f"http://{ip}/picoclaw_login" # TBD: Local server placeholder
    ...
    def on_rotate(self, delta):
        p = self.page
        if   p == PG_HOME:   self.home_sel    = (self.home_sel + delta) % len(HOME_TARGETS)
        elif p == PG_AGENT:  self.agent_scroll = max(0, self.agent_scroll + delta)
        elif p == PG_SETUP:  self.setup_sel    = (self.setup_sel + delta) % 8
    ...
    def on_press(self):
        ...
        elif p == PG_AUTH:   self._bg('auth', pc.auth_status)
    ...
    def _draw_auth(self, d):
        topbar(d, "Authentication")
        if self.auth_qr:
            img_qr = self.auth_qr.resize((100, 100))
            d.bitmap((LW//2-50, 30), img_qr)
        centered(d, "Scan QR to Login", LW//2, 140, FS, TEXT)
        centered(d, self.stats.get('ip',''), LW//2, 155, FN, DIM)

    def _draw_setup(self, d):
        topbar(d, "Setup")
        items = ["Mic Check", "Profile", "Model", "Onboard", "Gateway", "Clean", "Debug", "Back"]
        start = max(0, min(self.setup_sel-2, 4))
        for i, item in enumerate(items[start:start+5]):
            idx = i + start
            y = 26 + i*24
            sel = (idx == self.setup_sel)
            if sel: rrect(d, 4, y, LW-4, y+22, fill=SEL)
            d.text((12, y+5), item, font=FS, fill=TEXT if sel else DIM)

    def _draw_status(self, d):
        topbar(d, "System Status")
        y = 28
    ...
    def run(self):
        with TouchScreen() as touch, GpioKeys() as keys, RotaryEncoder() as rotary:
            while self.running:
                ...
                k = keys.read_event(0)
                if k and k[0]=='key_release': self.on_press()
                ...
                if t and t[0]=='touch_down':
                    ...
                    if tx < 50 and ty < 30: self.goto(PG_HOME)
